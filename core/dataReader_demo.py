...
class DataReader:
    ...
    def get_data(self, annotation_line):
        ...
        """
        line = annotation_line.numpy().decode().split()
    ...
    def get_mixup_data(self, annotation_line, hue=.1, sat=1.5, val=1.5):
        ...
        for i in range(2):
            line = annotation_line[i].numpy().decode().split()
            image_path = line[0]
            box = np.array([np.array(list(map(int, box.split(',')))) for box in line[1:]])
    ...
    def process_true_bbox(self, box_data):
        ...
        # 生成2种特征大小的网格
        grid_shapes = [input_shape // [32, 16][i] for i in range(len(cfg.anchor_masks))]
        # 创建2个特征大小的全零矩阵，[(b, 13, 13, 3, 25), ... , ...]存在列表中
    ...
    def parse(self, mode):
        def data_aug(annotation_line):
            if mode == "mosaic":
                image, bbox = tf.py_function(self.get_mosaic_data, [annotation_line], [tf.float32, tf.int32])
            elif mode == "mixup":
                image, bbox = tf.py_function(self.get_mixup_data, [annotation_line], [tf.float32, tf.int32])
            elif mode == "random":
                image, bbox = tf.py_function(self.get_random_data, [annotation_line], [tf.float32, tf.int32])
            else:
                image, bbox = tf.py_function(self.get_data, [annotation_line], [tf.float32, tf.int32])

            y13, y26 = tf.py_function(self.process_true_bbox, [bbox], [tf.float32, tf.float32])

            h, w = self.input_shape
            image.set_shape([h, w, 3])
            y13.set_shape([h // 32, w // 32, len(cfg.anchor_masks[0]), 5 + cfg.num_classes])
            y26.set_shape([h // 16, w // 16, len(cfg.anchor_masks[0]), 5 + cfg.num_classes])
            box_data = y13, y26

            return image, box_data
        return data_aug

    def make_datasets(self, annotation, mode):
        """
        ...
        mosaic_data = dataset.batch(4)
        mosaic_data = mosaic_data.map(self.parse('mosaic'), num_parallel_calls=tf.data.experimental.AUTOTUNE)

        mixup_data = dataset.batch(2)
        mixup_data = mixup_data.map(self.parse('mixup'), num_parallel_calls=tf.data.experimental.AUTOTUNE)

        random_data = dataset
        random_data = random_data.map(self.parse('random'), num_parallel_calls=tf.data.experimental.AUTOTUNE)

        normal_data = dataset
        normal_data = normal_data.map(self.parse('normal'), num_parallel_calls=tf.data.experimental.AUTOTUNE)

        if cfg.data_augmentation == 'mosaic':
            mosaic_data = mosaic_data.repeat().batch(self.batch_size)
            mosaic_data = mosaic_data.prefetch(1)

            return mosaic_data
        elif cfg.data_augmentation == 'mixup':
            mixup_data = mixup_data.repeat().shuffle(buffer_size=100).batch(self.batch_size)
            mixup_data = mixup_data.prefetch(1)

            return mixup_data
        elif cfg.data_augmentation == 'random':
            random_data = random_data.repeat().shuffle(buffer_size=100).batch(self.batch_size)
            random_data = random_data.prefetch(1)

            return random_data
        elif cfg.data_augmentation == 'normal' or mode == 'validation':
            normal_data = normal_data.repeat().shuffle(buffer_size=100).batch(self.batch_size)
            normal_data = normal_data.prefetch(1)

            return normal_data
        else:
            all_dataset = normal_data.concatenate(mosaic_data)
            all_dataset = all_dataset.concatenate(mixup_data)
            all_dataset = all_dataset.concatenate(random_data)
            all_dataset = all_dataset.concatenate(normal_data)

            all_dataset = all_dataset.shuffle(buffer_size=1).batch(self.batch_size)
            all_dataset = all_dataset.repeat().prefetch(1)
...
if __name__ == '__main__':
    reader = DataReader('../config/test.txt', cfg.input_shape, cfg.batch_size, cfg.data_augmentation)
    dataset = reader.make_datasets(reader.train_lines, 'train')
    for i, data in enumerate(dataset):
        img, bbox = data
        print(i, img.shape, bbox[0].shape)
