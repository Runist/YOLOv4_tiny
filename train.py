...
from nets.yolo import tiny_yolo_body
...
def main():
    ...
    train_data = tf.data.Dataset.from_generator(reader.generate,
                                                ...
                                                output_shapes=(image_shape, (y13_shape, y26_shape))
                                                )
...
def train_by_fit(model, train_datasets, valid_datasets, train_steps, valid_steps):
    ...
    """
    cbk = [
        callbacks.ModelCheckpoint('./model/yolov4_tiny_{val_loss:.04f}.h5', save_best_only=True, save_weights_only=True)
    ]
